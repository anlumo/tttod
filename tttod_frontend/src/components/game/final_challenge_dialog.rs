...
pub struct FinalChallengeDialog {
...
pub struct Props {
    ...
    pub offer_challenge: Callback<(Challenge, usize)>,
}
...
pub enum Msg {
    ...
    UpdateChosenClue(String),
}

impl Component for FinalChallengeDialog {
    type Message = Msg;
    ...
    fn update(&mut self, msg: Self::Message) -> ShouldRender {
        match msg {
            ...
            Msg::UpdateChosenClue(idx) => {
                if let Ok(idx) = idx.parse() {
                    self.chosen_clue = idx;
                }
                true
            }
        }
    ...
    fn view(&self) -> Html {
        ...
        let update_chosen_clue_callback = self.link.callback(Msg::UpdateChosenClue);
        ...
        html! {
            <ybc::ModalCard id={format!("offer-final-challenge-{}", player_id)} trigger={
                html! {
                ...
                html! {
                    <>
                        <div class="block is-size-5">
                            <p>{"The player is using the following secret:"}</p>
                            <ybc::Select classes="is-size-6" name="chosen-clue" value="" update=update_chosen_clue_callback>
                            {
                                for self.props.remaining_clues.iter().enumerate().map(|(idx, clue)| {
                                    html! {
                                        <option value=idx selected={idx == self.chosen_clue}>{clue}</option>
                                    }
                                })
                            }
                            </ybc::Select>
